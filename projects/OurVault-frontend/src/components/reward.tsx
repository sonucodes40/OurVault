  ...
  return (
    ...
    >
      ...
      >
        {/* FRONT */}
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center gap-3 p-4 rounded-xl border border-slate-800 bg-[#020617] ${
...
const MilestoneTrack = ({ savedAmount = 10 }: { savedAmount: number }) => {
  return (
                ...
                <div key={m.id} className="relative flex items-center gap-4">
                  ...
                  <div
                    className={`z-10 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-lg
                      ${done ? `${c.bg} ring-2 ${c.ring}` : "bg-slate-800"}`}
                  >
                  ...
                  <div className="flex flex-1 items-center justify-between">
                    ...
                    <span className={`rounded-full border px-2 py-0.5 text-xs font-medium
                      ${done ? `${c.border} ${c.text}` : "border-slate-700 text-slate-500"}`}>
...
export default function RewardsModal({ isOpen, onClose, totalSavings }: RewardsModalProps) {
  if (!isOpen) return null;

  const totalPoints = BADGES.filter(b => b.earned).reduce((sum, b) => sum + b.pts, 0);
  const earnedCount = BADGES.filter(b => b.earned).length;
  const nextMilestone = MILESTONES.find(m => totalSavings < m.threshold);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm sm:p-6">
      ...
      >
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-800 bg-[#0f172a]/95 p-6 backdrop-blur-md">
          ...
          <button 
            onClick={onClose}
            className="p-2 text-slate-400 hover:bg-slate-800 hover:text-white rounded-xl transition-colors cursor-pointer"
          >
            <X size={24} />
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
              <div className="flex items-center gap-3 border-b border-slate-800 p-4">
                ...
                <p className="font-semibold text-white">Total Points</p>
              </div>
              <div className="p-4 bg-[#020617]/50">
                <p className="text-3xl font-bold text-white">{totalPoints}</p>
                <span className="mt-2 flex w-fit items-center gap-x-2 rounded-full border border-amber-500/50 px-2 py-1 text-xs font-medium text-amber-400 bg-amber-500/10">
                  ⭐ Points Earned
                </span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
              <div className="flex items-center gap-3 border-b border-slate-800 p-4">
                ...
                <p className="font-semibold text-white">Badges</p>
              </div>
              <div className="p-4 bg-[#020617]/50">
                <p className="text-3xl font-bold text-white">
                  {earnedCount} <span className="text-base font-normal text-slate-500">/ {BADGES.length}</span>
                </p>
                <span className="mt-2 flex w-fit items-center gap-x-2 rounded-full border border-purple-500/50 px-2 py-1 text-xs font-medium text-purple-400 bg-purple-500/10">
                  🏅 Collected
                </span>
              </div>
            </div>
                  ...
                  <div className="h-2 w-full rounded-full bg-slate-800 overflow-hidden">
                    <div
                      className={`h-2 rounded-full transition-all duration-500 ${
                        progressToNext >= 100 ? "bg-green-400" :
                        progressToNext >= 50  ? "bg-yellow-400" : "bg-cyan-400"
                      }`}
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden mb-4">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  { icon: "💰", title: "Deposit ALGO",      desc: "Every deposit earns you points.",         pts: "+5 pts"   },
                  { icon: "🎯", title: "Hit Your Goal",     desc: "Reach your target for the Goal Crusher.", pts: "+50 pts"  },
                  { icon: "🔒", title: "Stay Consistent",   desc: "Keep funds locked for Vault Keeper.",     pts: "+20 pts"  },
                  { icon: "📈", title: "Save Milestones",   desc: "Hit thresholds for rewards.",             pts: "Up to 400" },
                ].map((tip, i) => (
                  <div
                    key={i}
                    className="flex flex-col gap-2 rounded-xl border border-slate-800 bg-[#020617] p-4 hover:border-slate-700 transition-colors"
                  >
